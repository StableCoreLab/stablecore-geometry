...
namespace Geometry
{
    TriangleMesh Tessellate(const SCPlaneSurface& surface, std::size_t uSegments, std::size_t vSegments)
    {
        if (!surface.IsValid())
        {
            return {};
        ...
        const auto vertexIndex = [uSegments](std::size_t u, std::size_t v) { return v * (uSegments + 1) + u; };
