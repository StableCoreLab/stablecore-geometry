...
namespace
{
    SCPoint3d SkewPoint(const SCPoint3d& point)
    {
        // Affine skew keeps each face planar while making the body
        // non-axis-aligned.
        return SCPoint3d{point.x + 0.3 * point.z, point.y + 0.2 * point.x, point.z};
    }

    SCPlane SkewPlane(const SCPlane& plane)
    {
        // For p' = A p (with A defined by SkewPoint), normal transforms as n' =
        // (A^{-1})^T n.
        const SCVector3d n = plane.normal;
        const SCVector3d transformedNormal{n.x - 0.2 * n.y, n.y, -0.3 * n.x + 0.06 * n.y + n.z};

        return SCPlane::FromPointAndNormal(SkewPoint(plane.origin), transformedNormal);
    }

    PolyhedronLoop3d SkewLoop(const PolyhedronLoop3d& loop)
    {
        std::vector<SCPoint3d> vertices;
        const std::vector<SCPoint3d>& source = loop.Vertices();
        vertices.reserve(source.size());
        for (const SCPoint3d& point : source)
        {
    ...
    SCPlane SupportPlaneFromLoop(const PolyhedronLoop3d& loop)
    {
        const std::vector<SCPoint3d>& vertices = loop.Vertices();
        if (vertices.size() < 3)
        {
            return SCPlane{};
        }

        const SCPoint3d& p0 = vertices[0];
        const SCPoint3d& p1 = vertices[1];
        const SCPoint3d& p2 = vertices[2];
        return SCPlane::FromPointAndNormal(p0, Cross(p1 - p0, p2 - p0));
    }
    ...
    PolyhedronBody BuildSupportPlaneMismatchedCubeBody()
    {
        try
        {
            ...
            faces.front() = PolyhedronFace3d(SCPlane::FromPointAndNormal(SCPoint3d{0.0, 0.0, 0.1}, SCVector3d{0.0, 0.0, 1.0}),
                                             first.OuterLoop());
    ...
    PolyhedronLoop3d TranslateLoop3d(const PolyhedronLoop3d& loop, const SCVector3d& delta)
    {
        try
        {
            std::vector<SCPoint3d> vertices;
    ...
    PolyhedronFace3d TranslateFace3d(const PolyhedronFace3d& face, const SCVector3d& delta)
    {
        try
        {
    ...
    PolyhedronBody BuildDeformedUnitCubeBody()
    {
        ...
        const SCPoint3d v0{0.1, 0.1, -0.1};  // displaced from (0,0,0)
        const SCPoint3d v1{1.0, 0.0, 0.0};
        const SCPoint3d v2{1.0, 1.0, 0.0};
        const SCPoint3d v3{0.0, 1.0, 0.0};
        const SCPoint3d v4{0.0, 0.0, 1.0};
        const SCPoint3d v5{1.0, 0.0, 1.0};
        const SCPoint3d v6{1.0, 1.0, 1.0};
        const SCPoint3d v7{0.0, 1.0, 1.0};

        // Support planes are the original axis-aligned planes of the unit cube
        // 闂?    // intentionally wrong for the three faces that contain v0.
        const SCPlane bottom{SCPoint3d{0.0, 0.0, 0.0}, SCVector3d{0.0, 0.0, -1.0}};
        const SCPlane top{SCPoint3d{0.0, 0.0, 1.0}, SCVector3d{0.0, 0.0, 1.0}};
        const SCPlane front{SCPoint3d{0.0, 0.0, 0.0}, SCVector3d{0.0, -1.0, 0.0}};
        const SCPlane back{SCPoint3d{0.0, 1.0, 0.0}, SCVector3d{0.0, 1.0, 0.0}};
        const SCPlane left{SCPoint3d{0.0, 0.0, 0.0}, SCVector3d{-1.0, 0.0, 0.0}};
        const SCPlane right{SCPoint3d{1.0, 0.0, 0.0}, SCVector3d{1.0, 0.0, 0.0}};
    ...
    // non-planar against their original support planes. This is a stronger
    // variant of the single-displaced-vertex deformed cube.
    PolyhedronBody BuildDualDeformedUnitCubeBody()
    {
        ...
        const SCPoint3d v4{0.0, 0.0, 1.0};
        const SCPoint3d v5{1.0, 0.0, 1.0};
        const SCPoint3d v6{0.9, 0.9, 1.1};  // displaced from (1,1,1)
        const SCPoint3d v7{0.0, 1.0, 1.0};
    ...
    PolyhedronBody BuildDeformedUnitCubeWithDuplicateLoopBody()
    {
        ...
        std::vector<SCPoint3d> loop = leftFace.OuterLoop().Vertices();
        if (loop.empty())
        {
            return PolyhedronBody();
        }
        ...
        faces[4] =
            PolyhedronFace3d(leftFace.SupportPlane(), PolyhedronLoop3d(std::move(loop)), CopyFaceHoles(leftFace));
        return PolyhedronBody(std::move(faces));
    }
    ...
    PolyhedronBody BuildDeformedUnitCubeWithDualDuplicateLoopBody()
    {
        ...
        std::vector<SCPoint3d> loop = frontFace.OuterLoop().Vertices();
        if (loop.empty())
        {
            return PolyhedronBody();
        }
        ...
        loop.insert(loop.begin(), loop.front());
        faces[2] =
            PolyhedronFace3d(frontFace.SupportPlane(), PolyhedronLoop3d(std::move(loop)), CopyFaceHoles(frontFace));
        return PolyhedronBody(std::move(faces));
    }
    ...
    PolyhedronBody BuildDualDeformedUnitCubeWithDualDuplicateLoopBody()
    {
        ...
        std::vector<SCPoint3d> loop = rightFace.OuterLoop().Vertices();
        if (loop.empty())
        {
            return PolyhedronBody();
        }
        ...
        loop.insert(loop.begin(), loop.front());
        faces[5] =
            PolyhedronFace3d(rightFace.SupportPlane(), PolyhedronLoop3d(std::move(loop)), CopyFaceHoles(rightFace));
        return PolyhedronBody(std::move(faces));
    }
    ...
    PolyhedronBody BuildMildlyNonPlanarCubeFaceBody()
    {
        ...
        std::vector<SCPoint3d> topVertices;
    ...
    PolyhedronBody BuildMildlyNonPlanarHoledFaceBody()
    {
        const PolyhedronLoop3d outer({
            ...
            SCPoint3d{4.0, 4.0, 0.0},
            SCPoint3d{0.0, 4.0, 0.0},
        });
        std::vector<SCPoint3d> holeVertices{
            SCPoint3d{1.0, 1.0, 0.0}, SCPoint3d{3.0, 1.0, 0.0}, SCPoint3d{3.0, 3.0, 0.0}, SCPoint3d{1.0, 3.0, 0.0}};
        holeVertices[2].z += 0.03;

        const PolyhedronFace3d face(SCPlane::FromPointAndNormal(SCPoint3d{0.0, 0.0, 0.0}, SCVector3d{0.0, 0.0, 1.0}),
                                    outer,
    ...
    {
        std::vector<SCPoint3d> outer{
            SCPoint3d{0.0, 0.0, 0.12}, SCPoint3d{6.0, 0.0, 0.00}, SCPoint3d{6.0, 6.0, 0.10}, SCPoint3d{0.0, 6.0, 0.00}};
        std::vector<SCPoint3d> hole{
            SCPoint3d{1.0, 1.0, 0.0}, SCPoint3d{5.0, 1.0, 0.0}, SCPoint3d{5.0, 5.0, 0.0}, SCPoint3d{1.0, 5.0, 0.0}};

        const PolyhedronFace3d face(SCPlane::FromPointAndNormal(SCPoint3d{0.0, 0.0, 0.2}, SCVector3d{0.0, 0.0, 1.0}),
    ...
    {
        const SCPlane mismatchedPlane = SCPlane::FromPointAndNormal(SCPoint3d{0.0, 0.0, 0.2}, SCVector3d{0.0, 0.0, 1.0});

        const SCPoint3d a{0.0, 0.0, 0.0};
        const SCPoint3d b{2.0, 0.0, 0.0};
        const SCPoint3d c{2.0, 2.0, 0.0};
        const SCPoint3d d{0.0, 2.0, 0.0};

        const SCPoint3d e{6.0, 0.0, 0.12};
        const SCPoint3d f{6.0, 2.0, 0.10};

        std::vector<SCPoint3d> hole{
            SCPoint3d{2.6, 0.4, 0.0}, SCPoint3d{5.4, 0.4, 0.0}, SCPoint3d{5.4, 1.6, 0.0}, SCPoint3d{2.6, 1.6, 0.0}};

        const PolyhedronFace3d faceA(mismatchedPlane, PolyhedronLoop3d({a, b, c, d}));
        const PolyhedronFace3d faceB(
            mismatchedPlane, PolyhedronLoop3d({b, e, f, c}), {PolyhedronLoop3d(std::move(hole))});
    ...
    PolyhedronBody BuildSharedChainHoleDominatedMixedContentBody()
    {
        ...
        const SCPoint3d g{8.0, 0.0, 0.0};
        const SCPoint3d h{8.0, 2.0, 0.0};
    ...
    PolyhedronBody BuildSupportMismatchNearEqualSharedChainHoleDominatedMixedContentBody()
    {
        ...
        const SCPoint3d b2{2.0 + dx, 0.0, 0.0};
        const SCPoint3d c2{2.0 + dx, 2.0, 0.0};
        const SCPoint3d e{6.0 + dx, 0.0, 0.12};
        const SCPoint3d f{6.0 + dx, 2.0, 0.10};

        const SCPoint3d e2{6.0, 0.0, 0.0};
        const SCPoint3d f2{6.0, 2.0, 0.0};
        const SCPoint3d g{8.0, 0.0, 0.0};
        const SCPoint3d h{8.0, 2.0, 0.0};
        ...
        const PolyhedronFace3d faceA(mismatchedPlane, PolyhedronLoop3d({a, b, c, d}));
        const PolyhedronFace3d faceB(
            mismatchedPlane, PolyhedronLoop3d({b2, e, f, c2}), {PolyhedronLoop3d(std::move(hole))});
    ...
    PolyhedronBody BuildSupportMismatchNearEqualSharedChainHoleDominatedMixedContentWithDuplicateHoleBody()
    {
        ...
        std::vector<SCPoint3d> hole{SCPoint3d{2.6, 0.4, 0.0},
                                  SCPoint3d{5.4, 0.4, 0.0},
                                  SCPoint3d{5.4, 0.4, 0.0},
                                  SCPoint3d{5.4, 1.6, 0.0},
                                  SCPoint3d{2.6, 1.6, 0.0}};
    ...
    PolyhedronBody BuildSupportMismatchNearEqualSharedChainHoleDominatedFullCompositionBody()
    {
        ...
        const SCPoint3d b2{2.0 + dx, 0.0, 0.0};
        const SCPoint3d c2{2.0 + dx, 2.0, 0.0};
        const SCPoint3d m{4.0 + dx, 0.0, 0.0};
        const SCPoint3d e{6.0 + dx, 0.0, 0.12};
        const SCPoint3d f{6.0 + dx, 2.0, 0.10};
        ...
        const PolyhedronFace3d faceA(mismatchedPlane, PolyhedronLoop3d({a, b, c, d}));
        const PolyhedronFace3d faceB(
            mismatchedPlane, PolyhedronLoop3d({b2, m, e, f, c2}), {PolyhedronLoop3d(std::move(hole))});
        const PolyhedronFace3d faceC(mismatchedPlane, PolyhedronLoop3d({e2, g, h, f2}));
        return PolyhedronBody({faceA, faceB, faceC});
    }
        ...
        const PolyhedronFace3d face(SCPlane::FromPointAndNormal(SCPoint3d{0.0, 0.0, 0.2}, SCVector3d{0.0, 0.0, 1.0}),
                                    ...
                                    }),
                                    {
                                        PolyhedronLoop3d({
                                            ...
                                            SCPoint3d{3.0, 1.0, 0.0},
                                            SCPoint3d{3.0, 3.0, 0.0},
                                            SCPoint3d{1.0, 3.0, 0.0},
                                        }),
                                    });
        return PolyhedronBody({face});
    }
    ...
    PolyhedronBody BuildCompositeRepairStressFaceBody()
    {
        ...
        const PolyhedronFace3d face(SCPlane::FromPointAndNormal(SCPoint3d{0.0, 0.0, 0.25}, SCVector3d{0.0, 0.0, 1.0}),
                                    PolyhedronLoop3d(std::move(outer)),
                                    {PolyhedronLoop3d(std::move(hole))});
        return PolyhedronBody({face});
    }
    ...
    PolyhedronBody BuildTinyScaleNonPlanarRepairBody()
    {
        const double s = 1e-5;
        std::vector<SCPoint3d> outer{
            SCPoint3d{0.0, 0.0, 0.0}, SCPoint3d{s, 0.0, 0.0}, SCPoint3d{s, s, 0.0}, SCPoint3d{0.0, s, 0.0}};
        // Small-scale non-planar disturbance that still describes a recoverable
        // loop.
        outer[2].z += 2e-6;

        const PolyhedronFace3d face(SCPlane::FromPointAndNormal(SCPoint3d{0.0, 0.0, 3e-6}, SCVector3d{0.0, 0.0, 1.0}),
                                    PolyhedronLoop3d(std::move(outer)));
        return PolyhedronBody({face});
    }
    ...
    PolyhedronBody BuildTinyScaleNonPlanarHoledRepairBody()
    {
        ...
        std::vector<SCPoint3d> outer{
            SCPoint3d{0.0, 0.0, 0.0}, SCPoint3d{s, 0.0, 0.0}, SCPoint3d{s, s, 0.0}, SCPoint3d{0.0, s, 0.0}};
        std::vector<SCPoint3d> hole{SCPoint3d{0.3 * s, 0.3 * s, 0.0},
                                  SCPoint3d{0.7 * s, 0.3 * s, 0.0},
                                  SCPoint3d{0.7 * s, 0.7 * s, 0.0},
                                  SCPoint3d{0.3 * s, 0.7 * s, 0.0}};

        outer[2].z += 2e-6;
        hole[1].z += 1.5e-6;

        const PolyhedronFace3d face(SCPlane::FromPointAndNormal(SCPoint3d{0.0, 0.0, 2e-6}, SCVector3d{0.0, 0.0, 1.0}),
                                    PolyhedronLoop3d(std::move(outer)),
                                    {PolyhedronLoop3d(std::move(hole))});
        return PolyhedronBody({face});
    }
    ...
    {
        const double s = 1e-5;
        std::vector<SCPoint3d> faceA{
            SCPoint3d{0.0, 0.0, 0.0}, SCPoint3d{s, 0.0, 0.0}, SCPoint3d{s, s, 0.0}, SCPoint3d{0.0, s, 0.0}};
        std::vector<SCPoint3d> faceB{
            SCPoint3d{2.0 * s, 0.0, 0.0}, SCPoint3d{3.0 * s, 0.0, 0.0}, SCPoint3d{3.0 * s, s, 0.0}, SCPoint3d{2.0 * s, s, 0.0}};

        faceA[2].z += 2e-6;
        faceB[1].z += 1.4e-6;

        return PolyhedronBody(
            {PolyhedronFace3d(SCPlane::FromPointAndNormal(SCPoint3d{0.0, 0.0, 2e-6}, SCVector3d{0.0, 0.0, 1.0}),
                              PolyhedronLoop3d(std::move(faceA))),
             PolyhedronFace3d(SCPlane::FromPointAndNormal(SCPoint3d{2.0 * s, 0.0, 2e-6}, SCVector3d{0.0, 0.0, 1.0}),
    ...
    PolyhedronBody BuildTinyScaleNonPlanarSharedEdgeBody()
    {
        const double s = 1e-5;
        const SCPoint3d a{0.0, 0.0, 0.0};
        const SCPoint3d b{s, 0.0, 0.0};
        const SCPoint3d c{s, s, 0.0};
        const SCPoint3d d{0.0, s, 1.8e-6};
        const SCPoint3d e{2.0 * s, 0.0, 1.4e-6};
        const SCPoint3d f{2.0 * s, s, 0.0};

        // Face A and Face B share edge (b, c), while each has tiny non-planar
        // drift.
        const PolyhedronFace3d faceA(SCPlane::FromPointAndNormal(SCPoint3d{0.0, 0.0, 2e-6}, SCVector3d{0.0, 0.0, 1.0}),
                                     PolyhedronLoop3d({a, b, c, d}));
        const PolyhedronFace3d faceB(SCPlane::FromPointAndNormal(SCPoint3d{s, 0.0, 2e-6}, SCVector3d{0.0, 0.0, 1.0}),
    ...
    PolyhedronBody BuildTinyScaleNonPlanarSharedEdgeChainBody()
    {
        ...
        const SCPoint3d b{s, 0.0, 0.0};
        const SCPoint3d c{s, s, 0.0};
        const SCPoint3d d{0.0, s, 1.5e-6};
        const SCPoint3d e{2.0 * s, 0.0, 1.1e-6};
        const SCPoint3d f{2.0 * s, s, 0.0};
        const SCPoint3d g{3.0 * s, 0.0, 1.4e-6};
        const SCPoint3d h{3.0 * s, s, 0.0};
        ...
        const PolyhedronFace3d faceB(SCPlane::FromPointAndNormal(SCPoint3d{s, 0.0, 2e-6}, SCVector3d{0.0, 0.0, 1.0}),
                                     PolyhedronLoop3d({b, e, f, c}));
        const PolyhedronFace3d faceC(SCPlane::FromPointAndNormal(SCPoint3d{2.0 * s, 0.0, 2e-6}, SCVector3d{0.0, 0.0, 1.0}),
    ...
    PolyhedronBody BuildTinyScaleSupportMismatchSharedEdgeChainBody()
    {
        ...
        const SCPoint3d b{s, 0.0, 0.0};
        const SCPoint3d c{s, s, 0.0};
        const SCPoint3d d{0.0, s, 1.6e-6};
        const SCPoint3d e{2.0 * s, 0.0, 1.1e-6};
        const SCPoint3d f{2.0 * s, s, 0.0};
        const SCPoint3d g{3.0 * s, 0.0, 1.4e-6};
        const SCPoint3d h{3.0 * s, s, 0.0};

        const SCPlane mismatchedPlane = SCPlane::FromPointAndNormal(SCPoint3d{0.0, 0.0, 4e-6}, SCVector3d{0.0, 0.0, 1.0});
    ...
    PolyhedronBody BuildTinyScaleNonPlanarSharedEdgeChainMixedContentBody()
    {
        ...
        const SCPoint3d g{3.0 * s, 0.0, 1.3e-6};
        const SCPoint3d h{3.0 * s, s, 0.0};

        std::vector<SCPoint3d> hole{SCPoint3d{1.2 * s, 0.3 * s, 0.0},
                                  SCPoint3d{1.8 * s, 0.3 * s, 1.0e-6},
                                  SCPoint3d{1.8 * s, 0.7 * s, 0.0},
                                  SCPoint3d{1.2 * s, 0.7 * s, 0.0}};
    ...
    PolyhedronBody BuildTinyScaleNonPlanarSharedEdgeChainWithDuplicateLoopBody()
    {
        ...
        const SCPoint3d b{s, 0.0, 0.0};
        const SCPoint3d c{s, s, 0.0};
        const SCPoint3d d{0.0, s, 1.4e-6};

        const SCPoint3d e{2.0 * s, 0.0, 1.2e-6};
        const SCPoint3d f{2.0 * s, s, 0.0};

        const SCPoint3d g{3.0 * s, 0.0, 1.1e-6};
        const SCPoint3d h{3.0 * s, s, 0.0};

        // Duplicate vertex in middle face loop to exercise normalization +
        // refit.
        const PolyhedronFace3d faceA(SCPlane::FromPointAndNormal(SCPoint3d{0.0, 0.0, 2e-6}, SCVector3d{0.0, 0.0, 1.0}),
                                     PolyhedronLoop3d({a, b, c, d}));
        const PolyhedronFace3d faceB(SCPlane::FromPointAndNormal(SCPoint3d{s, 0.0, 2e-6}, SCVector3d{0.0, 0.0, 1.0}),
    ...
    PolyhedronBody BuildTinyScaleSharedChainMixedContentWithCollinearLeadingBody()
    {
        ...
        const SCPoint3d e{2.0 * s, 0.0, 0.0};
        const SCPoint3d f{2.0 * s, s, 1.3e-6};
        const SCPoint3d g{3.0 * s, 0.0, 1.1e-6};
        const SCPoint3d h{3.0 * s, s, 0.0};

        // Leading points b -> m -> e are collinear to exercise robust normal
        // fallback while the loop remains mildly non-planar at tiny scale.
        const SCPoint3d m{1.5 * s, 0.0, 0.0};
    ...
    PolyhedronBody BuildTinyScaleSharedChainMixedContentSupportPlaneMismatchBody()
    {
        ...
        const SCPoint3d e{2.0 * s, 0.0, 1.2e-6};
        const SCPoint3d f{2.0 * s, s, 0.0};
        const SCPoint3d g{3.0 * s, 0.0, 1.0e-6};
        const SCPoint3d h{3.0 * s, s, 0.0};

        std::vector<SCPoint3d> hole{SCPoint3d{1.2 * s, 0.3 * s, 0.0},
                                  SCPoint3d{1.8 * s, 0.3 * s, 8e-7},
                                  SCPoint3d{1.8 * s, 0.7 * s, 0.0},
                                  SCPoint3d{1.2 * s, 0.7 * s, 0.0}};

        // Deliberately offset support planes to require refit in combination
        // with shared-chain mixed-content tiny-scale projection repair.
        const PolyhedronFace3d faceA(SCPlane::FromPointAndNormal(SCPoint3d{0.0, 0.0, 4e-6}, SCVector3d{0.0, 0.0, 1.0}),
                                     PolyhedronLoop3d({a, b, c, d}));
        const PolyhedronFace3d faceB(SCPlane::FromPointAndNormal(SCPoint3d{s, 0.0, 4e-6}, SCVector3d{0.0, 0.0, 1.0}),
                                     PolyhedronLoop3d({b, e, f, c}),
                                     {PolyhedronLoop3d(std::move(hole))});
        const PolyhedronFace3d faceC(SCPlane::FromPointAndNormal(SCPoint3d{2.0 * s, 0.0, 4e-6}, SCVector3d{0.0, 0.0, 1.0}),
                                     PolyhedronLoop3d({e, g, h, f}));
    ...
    PolyhedronBody BuildTinyScaleSharedChainSupportMismatchAndCollinearBody()
    {
        ...
        const SCPoint3d e{2.0 * s, 0.0, 0.0};
        const SCPoint3d f{2.0 * s, s, 1.2e-6};
        const SCPoint3d g{3.0 * s, 0.0, 1.1e-6};
        const SCPoint3d h{3.0 * s, s, 0.0};
        const SCPoint3d m{1.5 * s, 0.0, 0.0};
    ...
    {
        // A tiny-scale tetrahedron (4 triangular faces, 6 edges, 4 vertices)
        // where every support plane is identical and mismatched to force
        // per-face refit. All 6 edges are each shared by exactly two faces, so
        // after repair the resulting SCBrepBody should form a deterministically
        // closed shell.
        const double s = 1e-5;
        const SCPoint3d v0{0.0, 0.0, 0.0};
        const SCPoint3d v1{s, 0.0, 1.2e-6};
        const SCPoint3d v2{0.5 * s, 0.866 * s, 0.8e-6};
        const SCPoint3d v3{0.5 * s, 0.289 * s, 0.816 * s};

        // Deliberately flat support plane (z=2e-6, normal +z) while actual face
        // normals vary 闂?the per-face refit must compute the correct
        // orientation.
        const SCPlane mismatchedPlane = SCPlane::FromPointAndNormal(SCPoint3d{0.0, 0.0, 2e-6}, SCVector3d{0.0, 0.0, 1.0});
    ...
    PolyhedronBody BuildTinyScaleTriangularFaceChainBody()
    {
        ...
        // Key property: every shared vertex is one of the 3 defining vertices
        // of EVERY triangle that contains it, so after per-face refit its
        // distance to that face's refit-plane is exactly 0 闂?no projection
        // occurs and the original 3D position is preserved unchanged.
        // FindOrAddBrepVertex therefore merges all shared vertices exactly,
        // giving VertexCount=5, EdgeCount=7.
        const double s = 1e-5;
        const SCPoint3d a{0.0, 0.0, 0.0};
        const SCPoint3d b{s, 0.0, 1.3e-6};
        const SCPoint3d c{2.0 * s, 0.0, 0.9e-6};
        const SCPoint3d d{1.5 * s, s, 1.1e-6};
        const SCPoint3d e{3.0 * s, 0.5 * s, 0.7e-6};

        const SCPlane mismatchedPlane = SCPlane::FromPointAndNormal(SCPoint3d{0.0, 0.0, 2e-6}, SCVector3d{0.0, 0.0, 1.0});

        return PolyhedronBody({PolyhedronFace3d(mismatchedPlane, PolyhedronLoop3d({a, b, c})),
    ...
    PolyhedronBody BuildTinyScaleTriangularFanBody()
    {
        ...
        // Each radial edge (apex-vk) is shared by exactly two triangles, the
        // four outer edges (vk-v(k+1 mod 4)) are each used once. Key: apex is
        // one of the 3 defining vertices of all 4 triangles 闂?after per-face
        // refit its distance to each refit-plane is exactly 0 闂?apex is never
        // projected 闂?it is preserved at its original position in all faces
        // 闂?FindOrAddBrepVertex merges it exactly once.
        // Expected: VertexCount=5, EdgeCount=8 (4 radial + 4 outer).
        const double s = 1e-5;
        const SCPoint3d apex{0.0, 0.0, s};
        const SCPoint3d v0{s, 0.0, 0.0};
        const SCPoint3d v1{0.0, s, 0.0};
        const SCPoint3d v2{-s, 0.0, 0.0};
        const SCPoint3d v3{0.0, -s, 0.0};

        const SCPlane mismatchedPlane = SCPlane::FromPointAndNormal(SCPoint3d{0.0, 0.0, 2e-6}, SCVector3d{0.0, 0.0, 1.0});

        return PolyhedronBody({PolyhedronFace3d(mismatchedPlane, PolyhedronLoop3d({apex, v0, v1})),
    ...
    PolyhedronBody BuildNearEqualSharedEdgeQuadPairBody()
    {
        ...
        // Conversion should keep shared topology and place shared Brep vertices
        // at representative-id global average points instead of first-face
        // points.
        const double s = 1e-5;
        ...
        const SCPoint3d b{s, 0.0, 0.0};
        const SCPoint3d c{s, s, 0.0};
        const SCPoint3d d{0.0, s, 0.0};

        const SCPoint3d b2{s + dx, 0.0, 0.0};
        const SCPoint3d c2{s + dx, s, 0.0};
        const SCPoint3d e{2.0 * s, 0.0, 0.0};
        const SCPoint3d f{2.0 * s, s, 0.0};

        const SCPlane plane = SCPlane::FromPointAndNormal(SCPoint3d{0.0, 0.0, 0.0}, SCVector3d{0.0, 0.0, 1.0});
    ...
    {
        // Same near-equal shared-edge setup as
        // BuildNearEqualSharedEdgeQuadPairBody, but with intentionally
        // mismatched support planes to force repair path.
        const double s = 1e-5;
        const double dx = 2e-7;
        ...
        const SCPlane mismatched = SCPlane::FromPointAndNormal(SCPoint3d{0.0, 0.0, 3e-6}, SCVector3d{0.0, 0.0, 1.0});
        return PolyhedronBody({PolyhedronFace3d(mismatched, PolyhedronLoop3d({a, b, c, d})),
    ...
    PolyhedronBody BuildSupportMismatchNearEqualApexTriangularFanBody()
    {
        ...
        const double s = 1e-5;
        const SCPoint3d apex0{0.0, 0.0, s};
        const SCPoint3d apex1{1.0e-7, 0.0, s};
        const SCPoint3d apex2{2.0e-7, 0.0, s};
        const SCPoint3d apex3{-1.0e-7, 0.0, s};
        ...
        return PolyhedronBody({PolyhedronFace3d(mismatched, PolyhedronLoop3d({apex0, v0, v1})),
    ...
    {
        // Three quads in a shared-edge chain with near-equal (<eps)
        // perturbations on middle-face shared vertices. Mismatched support
        // planes force repair.
        const double s = 1e-5;
        const double dx = 2e-7;
        ...
        const SCPoint3d b2{s + dx, 0.0, 0.0};
        const SCPoint3d c2{s + dx, s, 0.0};
        const SCPoint3d e{2.0 * s + dx, 0.0, 0.0};
        const SCPoint3d f{2.0 * s + dx, s, 0.0};

        const SCPoint3d e2{2.0 * s, 0.0, 0.0};
        const SCPoint3d f2{2.0 * s, s, 0.0};
        const SCPoint3d g{3.0 * s, 0.0, 0.0};
        const SCPoint3d h{3.0 * s, s, 0.0};

        const SCPlane mismatched = SCPlane::FromPointAndNormal(SCPoint3d{0.0, 0.0, 4e-6}, SCVector3d{0.0, 0.0, 1.0});
    ...
    PolyhedronBody BuildSupportMismatchNearEqualSharedCornerQuadFanBody()
    {
        ...
        const SCPoint3d p0a{0.0, 0.0, 0.0};
        const SCPoint3d p0b{1.0e-7, 0.0, 0.0};
        const SCPoint3d p0c{-1.0e-7, 0.0, 0.0};

        const SCPoint3d a1{s, 0.0, 0.0};
        const SCPoint3d a2{s, s, 0.0};
        const SCPoint3d a3{0.0, s, 0.0};

        const SCPoint3d b1{-s, 0.0, 0.0};
        const SCPoint3d b2{-s, s, 0.0};
        const SCPoint3d b3{0.0, s, 0.0};

        const SCPoint3d c1{0.0, -s, 0.0};
        const SCPoint3d c2{s, -s, 0.0};
        const SCPoint3d c3{s, 0.0, 0.0};

        const SCPlane mismatched = SCPlane::FromPointAndNormal(SCPoint3d{0.0, 0.0, 4e-6}, SCVector3d{0.0, 0.0, 1.0});

        return PolyhedronBody({PolyhedronFace3d(mismatched, PolyhedronLoop3d({p0a, a1, a2, a3})),
    ...
    PolyhedronBody BuildSupportMismatchNearEqualClosedTetrahedronBody()
    {
        ...
        // (<eps) perturbations across faces. Mismatched support planes force
        // repair before conversion. Representative averaging should merge the
        // perturbed variants into one shared Brep vertex.
        const double s = 1e-5;
        const SCPoint3d v0a{0.0, 0.0, 0.0};
        const SCPoint3d v0b{1.0e-7, 0.0, 0.0};
        const SCPoint3d v0c{-1.0e-7, 0.0, 0.0};
    ...
    {
        // Shared-edge chain mixed-content variant with near-equal perturbations
        // on shared vertices and duplicate hole vertices on middle face.
        const double s = 1e-5;
        const double dx = 2e-7;
        ...
        const SCPoint3d e{2.0 * s + dx, 0.0, 1.2e-6};
        const SCPoint3d f{2.0 * s + dx, s, 0.0};
        const SCPoint3d g{3.0 * s, 0.0, 1.0e-6};
        const SCPoint3d h{3.0 * s, s, 0.0};

        const SCPoint3d e2{2.0 * s, 0.0, 1.2e-6};
        const SCPoint3d f2{2.0 * s, s, 0.0};
        ...
        return PolyhedronBody(
            {PolyhedronFace3d(mismatched, PolyhedronLoop3d({a, b, c, d})),
             PolyhedronFace3d(mismatched,
                              PolyhedronLoop3d({b + SCVector3d{dx, 0.0, 0.0}, e, f, c + SCVector3d{dx, 0.0, 0.0}}),
                              {PolyhedronLoop3d(std::move(hole))}),
             PolyhedronFace3d(mismatched, PolyhedronLoop3d({e2, g, h, f2}))});
    }
    ...
    {
        // Shared-chain full composition: support-plane mismatch +
        // collinear-leading
        // + duplicate-hole normalization + near-equal shared-edge
        // perturbations.
        const double s = 1e-5;
        const double dx = 2e-7;
        ...
        const SCPoint3d e{2.0 * s + dx, 0.0, 0.0};
        const SCPoint3d f{2.0 * s + dx, s, 1.2e-6};
        const SCPoint3d g{3.0 * s, 0.0, 1.1e-6};
        const SCPoint3d h{3.0 * s, s, 0.0};
        const SCPoint3d m{1.5 * s + dx, 0.0, 0.0};

        const SCPoint3d e2{2.0 * s, 0.0, 0.0};
        const SCPoint3d f2{2.0 * s, s, 1.2e-6};
        ...
        return PolyhedronBody(
            {PolyhedronFace3d(mismatched, PolyhedronLoop3d({a, b, c, d})),
             PolyhedronFace3d(mismatched,
                              PolyhedronLoop3d({b + SCVector3d{dx, 0.0, 0.0}, m, e, f, c + SCVector3d{dx, 0.0, 0.0}}),
                              {PolyhedronLoop3d(std::move(hole))}),
             PolyhedronFace3d(mismatched, PolyhedronLoop3d({e2, g, h, f2}))});
    }
    ...
    {
        // Closed tetrahedron where two logical shared vertices (v0, v1) are
        // each represented by near-equal (<eps) variants across faces.
        const double s = 1e-5;
        ...
        const SCPoint3d v1a{s + 2.0e-7, 0.0, 1.2e-6};
        const SCPoint3d v1b{s - 1.0e-7, 0.0, 1.2e-6};
        const SCPoint3d v1c{s, 0.0, 1.2e-6};
    ...
    PolyhedronBody BuildSupportMismatchNearEqualClosedTetrahedronDualVerticesWithDuplicateLoopBody()
    {
        ...
        std::vector<SCPoint3d> loop = face.OuterLoop().Vertices();
        if (loop.empty())
        {
            return PolyhedronBody();
        }
    ...
    PolyhedronBody BuildSupportMismatchNearEqualClosedTetrahedronAllVerticesBody()
    {
        ...
        // near-equal (<eps) position variants across their three incident
        // faces, exercising simultaneous representative-average placement for
        // every shared vertex at once.
        const double s = 1e-5;
        ...
        const SCPoint3d v2a{0.5 * s + 1.0e-7, 0.866 * s, 0.8e-6};
        const SCPoint3d v2b{0.5 * s - 1.0e-7, 0.866 * s, 0.8e-6};
        const SCPoint3d v2c{0.5 * s, 0.866 * s + 1.0e-7, 0.8e-6};
        // avg x = 0.5*s, avg y = 0.866*s + 1e-7/3

        // v3 near {0.5*s, 0.289*s, 0.816*s}: faces 1,2,3
        const SCPoint3d v3a{0.5 * s + 2.0e-7, 0.289 * s, 0.816 * s};
        const SCPoint3d v3b{0.5 * s - 1.0e-7, 0.289 * s, 0.816 * s};
        const SCPoint3d v3c{0.5 * s, 0.289 * s, 0.816 * s};
        // avg x = 0.5*s + 1e-7/3

        const SCPlane mismatched = SCPlane::FromPointAndNormal(SCPoint3d{0.0, 0.0, 3e-6}, SCVector3d{0.0, 0.0, 1.0});

        return PolyhedronBody({PolyhedronFace3d(mismatched, PolyhedronLoop3d({v0a, v1a, v2a})),
    ...
    PolyhedronBody BuildSupportMismatchNearEqualClosedPrismDualVerticesBody()
    {
        ...
        const SCPoint3d v0a{1.0e-7, 0.0, 0.0};
        const SCPoint3d v0b{-1.0e-7, 0.0, 0.0};
        const SCPoint3d v0c{0.0, 0.0, 0.0};
        // avg x = 0

        const SCPoint3d v1{s, 0.0, 0.0};
        const SCPoint3d v2{0.5 * s, 0.866 * s, 0.0};
        const SCPoint3d v3{0.0, 0.0, s};

        // v4 near {s,0,s}: top + front-side + right-side
        const SCPoint3d v4a{s + 2.0e-7, 0.0, s};
        const SCPoint3d v4b{s - 1.0e-7, 0.0, s};
        const SCPoint3d v4c{s, 0.0, s};
        // avg x = s + 1e-7/3

        const SCPoint3d v5{0.5 * s, 0.866 * s, s};
    ...
    {
        // Reuse dual-vertices near-equal closed-prism and inject one
        ...
        std::vector<SCPoint3d> loop = sideFace.OuterLoop().Vertices();
        if (loop.empty())
        {
            return PolyhedronBody();
        }
    ...
    PolyhedronBody BuildSupportMismatchNearEqualClosedPrismDualVerticesWithDualDuplicateLoopBody()
    {
        ...
        // variant and inject another duplicate leading vertex on a second side
        // face.
    ...
    {
        // Closed triangular prism where all six logical shared vertices each
        // have near-equal variants across their incident faces.
        const double s = 1e-5;
        ...
        const SCPoint3d v1a{s + 2.0e-7, 0.0, 0.0};
        const SCPoint3d v1b{s - 1.0e-7, 0.0, 0.0};
        const SCPoint3d v1c{s, 0.0, 0.0};

        const SCPoint3d v2a{0.5 * s + 1.0e-7, 0.866 * s, 0.0};
        const SCPoint3d v2b{0.5 * s - 1.0e-7, 0.866 * s, 0.0};
        const SCPoint3d v2c{0.5 * s, 0.866 * s + 1.0e-7, 0.0};

        const SCPoint3d v3a{0.0, 0.0, s};
        const SCPoint3d v3b{1.0e-7, 0.0, s};
        const SCPoint3d v3c{-1.0e-7, 0.0, s};

        const SCPoint3d v4a{s + 3.0e-7, 0.0, s};
        const SCPoint3d v4b{s - 2.0e-7, 0.0, s};
        const SCPoint3d v4c{s + 1.0e-7, 0.0, s};

        const SCPoint3d v5a{0.5 * s, 0.866 * s, s};
        const SCPoint3d v5b{0.5 * s + 1.0e-7, 0.866 * s, s};
        const SCPoint3d v5c{0.5 * s - 1.0e-7, 0.866 * s, s};
    ...
    PolyhedronBody BuildSupportMismatchNearEqualClosedPrismAllVerticesWithDuplicateLoopBody()
    {
        ...
        const PolyhedronFace3d sideFace = faces[2];
        std::vector<SCPoint3d> vertices = sideFace.OuterLoop().Vertices();
        if (vertices.empty())
        {
            return PolyhedronBody();
        }
    ...
    {
        // Tiny-scale 2闂?闂? rectangular box (pure-quad closed shell) where all
        // 8 logical shared vertices have near-equal variants across their 3
        // incident faces. All faces use a single mismatched support plane.
        // Tests that representative-average vertex placement works for a
        // pure-quad closed-shell topology with non-equal side lengths. Expected
        // after repair: FaceCount=6 / VertexCount=8 / EdgeCount=12 /
        // IsClosed=true.
        const double s = 1e-5;
        const double w = 2.0 * s;  // width in x-direction (twice the other dimensions)

        // v0 闂?(0,0,0): bottom + front + left
        const SCPoint3d v0a{1.0e-7, 0.0, 0.0};
        const SCPoint3d v0b{-1.0e-7, 0.0, 0.0};
        const SCPoint3d v0c{0.0, 0.0, 0.0};

        // v1 闂?(w,0,0): bottom + front + right
        const SCPoint3d v1a{w + 2.0e-7, 0.0, 0.0};
        const SCPoint3d v1b{w - 1.0e-7, 0.0, 0.0};
        const SCPoint3d v1c{w + 1.0e-7, 0.0, 0.0};

        // v2 闂?(w,s,0): bottom + back + right
        const SCPoint3d v2a{w + 1.0e-7, s, 0.0};
        const SCPoint3d v2b{w, s + 1.0e-7, 0.0};
        const SCPoint3d v2c{w, s, 0.0};

        // v3 闂?(0,s,0): bottom + back + left
        const SCPoint3d v3a{0.0, s + 2.0e-7, 0.0};
        const SCPoint3d v3b{0.0, s, 0.0};
        const SCPoint3d v3c{-1.0e-7, s, 0.0};

        // v4 闂?(0,0,s): top + front + left
        const SCPoint3d v4a{0.0, 0.0, s};
        const SCPoint3d v4b{1.0e-7, 0.0, s};
        const SCPoint3d v4c{0.0, 0.0, s + 1.0e-7};

        // v5 闂?(w,0,s): top + front + right
        const SCPoint3d v5a{w + 1.0e-7, 0.0, s};
        const SCPoint3d v5b{w, 0.0, s - 1.0e-7};
        const SCPoint3d v5c{w, 0.0, s};

        // v6 闂?(w,s,s): top + back + right
        const SCPoint3d v6a{w, s, s + 1.0e-7};
        const SCPoint3d v6b{w + 1.0e-7, s, s};
        const SCPoint3d v6c{w, s + 1.0e-7, s};

        // v7 闂?(0,s,s): top + back + left
        const SCPoint3d v7a{0.0, s, s};
        const SCPoint3d v7b{-1.0e-7, s, s};
        const SCPoint3d v7c{0.0, s, s + 1.0e-7};

        const SCPlane mismatched = SCPlane::FromPointAndNormal(SCPoint3d{0.0, 0.0, 3e-6}, SCVector3d{0.0, 0.0, 1.0});

        return PolyhedronBody({// Bottom (z闂?, normal -z): v0, v3, v2, v1
                               PolyhedronFace3d(mismatched, PolyhedronLoop3d({v0a, v3a, v2a, v1a})),
                               // Top (z闂備焦鍓氶崰鏍ь熆? normal +z): v4, v5, v6, v7
                               PolyhedronFace3d(mismatched, PolyhedronLoop3d({v4a, v5a, v6a, v7a})),
                               // Front (y闂?, normal -y): v0, v1, v5, v4
                               PolyhedronFace3d(mismatched, PolyhedronLoop3d({v0b, v1b, v5b, v4b})),
                               // Back (y闂備焦鍓氶崰鏍ь熆? normal +y): v3, v7, v6, v2
                               PolyhedronFace3d(mismatched, PolyhedronLoop3d({v3b, v7b, v6b, v2b})),
                               // Left (x闂?, normal -x): v0, v4, v7, v3
                               PolyhedronFace3d(mismatched, PolyhedronLoop3d({v0c, v4c, v7c, v3c})),
                               // Right (x闂備焦鍓氶崰鏍ь熆? normal +x): v1, v2, v6, v5
    ...
    PolyhedronBody BuildSupportMismatchNearEqualClosedCuboidAllVerticesWithDuplicateLoopBody()
    {
        ...
        std::vector<SCPoint3d> loop = front.OuterLoop().Vertices();
        if (loop.empty())
        {
            return PolyhedronBody();
        }
    ...
    {
        // Start from the single-duplicate variant and add another duplicate
        // leading vertex on a second face, to stress duplicate-loop
        // normalization composition in a closed all-vertices near-equal cuboid
        // input.
        ...
        std::vector<SCPoint3d> loop = back.OuterLoop().Vertices();
        if (loop.empty())
        {
            return PolyhedronBody();
        }
    ...
    PolyhedronBody BuildSupportMismatchNearEqualClosedCuboidDualVerticesWithDuplicateLoopBody()
    {
        ...
        // Stable vertices.
        const SCPoint3d v1{w, 0.0, 0.0};
        const SCPoint3d v2{w, s, 0.0};
        const SCPoint3d v3{0.0, s, 0.0};
        const SCPoint3d v4{0.0, 0.0, s};
        const SCPoint3d v5{w, 0.0, s};
        const SCPoint3d v7{0.0, s, s};

        // v6 variants: top/back/right
        const SCPoint3d v6a{w + 2.0e-7, s, s};
        const SCPoint3d v6b{w - 1.0e-7, s, s};
        const SCPoint3d v6c{w, s, s};

        const SCPlane mismatched = SCPlane::FromPointAndNormal(SCPoint3d{0.0, 0.0, 3e-6}, SCVector3d{0.0, 0.0, 1.0});

        std::vector<SCPoint3d> leftLoop{v0c, v4, v7, v3};
        leftLoop.insert(leftLoop.begin(), leftLoop.front());

        return PolyhedronBody({// Bottom
    ...
    PolyhedronBody BuildSupportMismatchNearEqualClosedCuboidDualVerticesBody()
    {
        ...
        return PolyhedronBody({PolyhedronFace3d(mismatched, PolyhedronLoop3d({v0a, v3, v2, v1})),
                               PolyhedronFace3d(mismatched, PolyhedronLoop3d({v4, v5, v6a, v7})),
                               PolyhedronFace3d(mismatched, PolyhedronLoop3d({v0b, v1, v5, v4})),
                               PolyhedronFace3d(mismatched, PolyhedronLoop3d({v3, v7, v6b, v2})),
    ...
    PolyhedronBody BuildSupportMismatchNearEqualClosedCuboidTripleVerticesBody()
    {
        ...
        const SCPoint3d v1a{w + 1.0e-7, 0.0, 0.0};
        const SCPoint3d v1b{w - 1.0e-7, 0.0, 0.0};
        const SCPoint3d v1c{w, 0.0, 0.0};
        ...
        return PolyhedronBody({PolyhedronFace3d(mismatched, PolyhedronLoop3d({v0a, v3, v2, v1a})),
                               PolyhedronFace3d(mismatched, PolyhedronLoop3d({v4, v5, v6a, v7})),
    ...
    // Same triple-vertices near-equal perturbation, but the left face also has
    // a duplicate leading vertex injected for duplicate-loop-normalization
    // stress.
    ...
    {
        // Start from the single-duplicate triple-vertices variant and add a
        // second duplicate leading vertex on another face to stress composition
        // behavior.
    ...
    void AssertClosedCuboidAllVerticesRepresentativeTargets(const SCBrepBody& body)
    {
        const double s = 1e-5;
        const double w = 2.0 * s;
        const double third = 1.0e-7 / 3.0;
        const std::vector<SCPoint3d> expectedPoints{SCPoint3d{0.0, 0.0, 0.0},
                                                  SCPoint3d{w + 2.0 * third, 0.0, 0.0},
                                                  SCPoint3d{w + third, s + third, 0.0},
                                                  SCPoint3d{-third, s + 2.0 * third, 0.0},
                                                  SCPoint3d{third, 0.0, s + third},
                                                  SCPoint3d{w + third, 0.0, s - third},
                                                  SCPoint3d{w + third, s + third, s + third},
                                                  SCPoint3d{-third, s, s + third}};
        ...
        {
            const SCPoint3d point = body.VertexAt(i).Point();
            bool matched = false;
            for (std::size_t expectedIndex = 0; expectedIndex < expectedPoints.size(); ++expectedIndex)
            {
                const SCPoint3d& expected = expectedPoints[expectedIndex];
                if (std::abs(point.x - expected.x) < 1e-10 && std::abs(point.y - expected.y) < 1e-10 &&
...
// to a TriangleMesh with the correct triangle count and total ISCSurface area.
// High-fidelity feature-preserving conversion and non-planar polyhedron闂備焦鍓氶崑鍛櫠閻氱p
...
TEST(Conversion3dCapabilityTest, UnitCubePolyhedronBodyConvertsToTriangleMesh)
{
    ...
    // Each of the 6 quad faces is split into 2 triangles 闂?12 total
    ASSERT_EQ(result.mesh.TriangleCount(), 12);
    // Total ISCSurface area of a unit cube = 6 闂?(1闂?) = 6.0
...
// SCBrepBody representation while preserving top-level face cardinality.
...
TEST(Conversion3dCapabilityTest, NearEqualSharedEdgeVerticesUseRepresentativeAverageTarget)
{
    ...
    {
        const SCPoint3d point = result.body.VertexAt(i).Point();
...
// across quad and triangle faces simultaneously converge to stable average
// targets.
...
// converted to SCBrepBody without requiring robust non-planar repair.
...
// produce a valid SCBrepBody for a recoverable polyhedron input.
...
// SCBrepBody. The area-based triplet selection naturally ignores the displaced
...
// and conversion still repairs to a closed shared-topology SCBrepBody.
...
// refit-plane conversion to SCBrepBody.
...
// adjacency chain where local refit is applied across multiple neighboring
// faces. The conversion now preserves source representative vertex identities
// across repaired faces, so shared-edge chain topology remains deterministic
// even when per-face projection introduces small geometric drift.
...
// closed SCBrepBody with globally shared topology (VertexCount=4, EdgeCount=6,
...
// that face's refit-plane is exactly 0 闂?no projection divergence.  The
...
// Demonstrates planar holed SCBrepBody conversion keeps representative area by
...
// SCBrepBody with exactly VertexCount=5 and EdgeCount=8. The shared apex is a
// defining vertex of all four triangles 闂?distance 0 from each refit-plane
// 闂?// never projected 闂?exact merge.  Four radial edges are each used twice;
// four outer edges are each used once 闂?the open boundary is deterministic.
...
TEST(Conversion3dCapabilityTest, PlanarHoledBrepBodyConvertsToMeshWithExpectedArea)
{
    ...
    auto supportSurface = std::shared_ptr<ISCSurface>(planeSurface.Clone().release());
    const SCCurveOnSurface outerTrim(supportSurface,
                                   SCPolyline2d(
                                       {
                                           SCPoint2d{0.0, 0.0},
                                           SCPoint2d{4.0, 0.0},
    ...
    const SCCurveOnSurface holeTrim(supportSurface,
                                  SCPolyline2d(
                                      {
                                          SCPoint2d{1.0, 1.0},
                                          SCPoint2d{3.0, 1.0},
    ...
    const SCBrepFace face(
        std::shared_ptr<ISCSurface>(planeSurface.Clone().release()), outerLoop, {holeLoop}, outerTrim, {holeTrim});
...
// Demonstrates planar multi-face SCBrepBody conversion keeps representative
...
TEST(Conversion3dCapabilityTest, PlanarMultiFaceBrepBodyConvertsToMeshWithExpectedArea)
{
    ...
    const SCPlaneSurface planeSurface =
        SCPlaneSurface::FromPlane(SCPlane::FromPointAndNormal(SCPoint3d{0.0, 0.0, 0.0}, SCVector3d{0.0, 0.0, 1.0}));
    auto supportA = std::shared_ptr<ISCSurface>(planeSurface.Clone().release());
    auto supportB = std::shared_ptr<ISCSurface>(planeSurface.Clone().release());

    const SCCurveOnSurface trimA(supportA,
                               SCPolyline2d(
                                   {
                                       SCPoint2d{0.0, 0.0},
                                       SCPoint2d{2.0, 0.0},
                                   ...
                                   SCPolylineClosure::Closed));
    const SCCurveOnSurface trimB(supportB,
                               SCPolyline2d(
                                   {
                                       SCPoint2d{3.0, 0.0},
                                       SCPoint2d{4.0, 0.0},
    ...
    const SCBrepFace faceA(std::shared_ptr<ISCSurface>(planeSurface.Clone().release()), outerA, {}, trimA, {});
    const SCBrepFace faceB(std::shared_ptr<ISCSurface>(planeSurface.Clone().release()), outerB, {}, trimB, {});
    const SCBrepBody body(vertices, edges, {SCBrepShell({faceA, faceB}, false)});
    ASSERT_TRUE(body.IsValid());
...
TEST(Conversion3dCapabilityTest, PlanarSharedEdgeBrepBodyConvertsToMeshWithSharedVertices)
{
    ...
    auto makeTrim = [&](const std::vector<SCPoint2d>& points) {
        return SCCurveOnSurface(std::shared_ptr<ISCSurface>(planeSurface.Clone().release()),
                              SCPolyline2d(points, SCPolylineClosure::Closed));
    };

    const SCBrepFace faceA(std::shared_ptr<ISCSurface>(planeSurface.Clone().release()),
                         outerA,
                         {},
                         makeTrim({
                             SCPoint2d{0.0, 0.0},
                         ...
                         {});
    const SCBrepFace faceB(std::shared_ptr<ISCSurface>(planeSurface.Clone().release()),
                         outerB,
                         {},
                         makeTrim({
                             SCPoint2d{1.0, 0.0},
...
{
    std::vector<SCBrepVertex> vertices{// Face A outer square (area 16)
                                     SCBrepVertex(SCPoint3d{0.0, 0.0, 0.0}),
                                     SCBrepVertex(SCPoint3d{4.0, 0.0, 0.0}),
                                     SCBrepVertex(SCPoint3d{4.0, 4.0, 0.0}),
                                     SCBrepVertex(SCPoint3d{0.0, 4.0, 0.0}),
    ...
    const SCBrepLoop outerA({SCBrepCoedge(0, false), SCBrepCoedge(1, false), SCBrepCoedge(2, false), SCBrepCoedge(3, false)});
    const SCBrepLoop holeA({SCBrepCoedge(4, false), SCBrepCoedge(5, false), SCBrepCoedge(6, false), SCBrepCoedge(7, false)});
    const SCBrepLoop outerB({SCBrepCoedge(8, false), SCBrepCoedge(9, false), SCBrepCoedge(10, false), SCBrepCoedge(11, false)});
    ...
    const SCCurveOnSurface outerTrimA = makeTrim({
        SCPoint2d{0.0, 0.0},
        SCPoint2d{4.0, 0.0},
        SCPoint2d{4.0, 4.0},
        SCPoint2d{0.0, 4.0},
    });
    const SCCurveOnSurface holeTrimA = makeTrim({
        SCPoint2d{1.0, 1.0},
        SCPoint2d{3.0, 1.0},
        SCPoint2d{3.0, 3.0},
        SCPoint2d{1.0, 3.0},
    });
    const SCCurveOnSurface outerTrimB = makeTrim({
        SCPoint2d{5.0, 0.0},
        SCPoint2d{6.0, 0.0},
        SCPoint2d{6.0, 2.0},
        SCPoint2d{5.0, 2.0},
    ...
    const SCBrepFace faceA(
        std::shared_ptr<ISCSurface>(planeSurface.Clone().release()), outerA, {holeA}, outerTrimA, {holeTrimA});
    const SCBrepFace faceB(std::shared_ptr<ISCSurface>(planeSurface.Clone().release()), outerB, {}, outerTrimB, {});
    const SCBrepBody body(vertices, edges, {SCBrepShell({faceA, faceB}, false)});
    ASSERT_TRUE(body.IsValid());
