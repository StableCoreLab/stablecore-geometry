...
| tests/capabilities/test_3d_healing.cpp | Healing3dCapabilityTest | Heal(PolyhedronBody) 幂等 6 faces + Heal(BrepBody) trim 回填（含 holed face） + Aggressive 最小闭壳子策略 |
...
| Healing3dGapTest::AggressiveShellRepairPolicyRemainsOpen | 激进 shell 修复策略（超出 single-face planar closure 子策略） |
...
- P3-D：`Heal(..., policy=Aggressive)` 已转正 open planar single-face shell 的确定性闭壳子策略
